<template>
  <f7-page name="training-session">
    ...
    <template v-else-if="session">
      ...
      <div v-for="item in session.items" :key="item.id" class="item" :class="{ 'item--done': item.result }">
        ...
        <div v-if="editing === item.id" class="log">
          ...
          <div class="log__actions">
            <button class="log__btn log__btn--save" @click="save(item)">{{ t('training.save') }}</button>
            <button class="log__btn" @click="skip(item)">{{ t('training.skip') }}</button>
            <button class="log__btn" @click="editing = null">{{ t('training.cancel') }}</button>
          </div>
        </div>
