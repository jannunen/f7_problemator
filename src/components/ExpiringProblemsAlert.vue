<template>
  <div v-if="expiringProblems.length > 0" class="p-banner p-banner--warning mx-4 my-2">
    <span class="material-icons p-banner__icon p-text-warning">warning</span>
    <div class="p-banner__content">
      ...
      <div v-if="showDetails" class="mt-3">
        <div class="grid grid-cols-6 text-xs font-semibold mb-1 p-text-muted">
          <span>Problem</span>
          <span>Wall</span>
          <span>Grade</span>
          <span>Colour</span>
        ...
        <div v-for="problem in expiringProblems" :key="problem.id" class="expiring-row grid grid-cols-6 items-center py-1 text-sm">
          ...
          <span v-else class="col-span-2 text-xs p-text-muted">Soon</span>
        </div>
      </div>
    </div>
  </div>
